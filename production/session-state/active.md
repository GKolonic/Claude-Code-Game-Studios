...
Task: HUD & Progress System GDD — Designed
<!-- /STATUS -->
...
- ⚠️ **Pre-existing inconsistency flagged for `/consistency-check`** (NOT edited): `RivalFaithConfig.aggression_interval_turns` default is **6** in game-config.md vs **3** in rival-faith-system.md — the two GDDs must agree before implementation
...
- design/registry/entities.yaml — HUDConfig constant + F1/F2/F3 formulas + referenced_by updates
...
- ⬜ **Flagged for `/consistency-check`** (NOT edited): `RivalFaithConfig.aggression_interval_turns` default 6 (game-config.md) vs 3 (rival-faith-system.md)

## Next
Pre-production design documentation is COMPLETE — all 14/14 MVP GDDs designed.
Remaining pre-production items:
1. **Architecture plan** — run `/architecture-plan` or `/architecture-review` over the 14 GDDs (data structures, interfaces, Autoload/scene ownership, dependency graph)
2. **Sprint plan** — break MVP into implementation sprints (foundation → core → feature → presentation)
3. **Gate-check** — run `/gate-check pre-production` (validates GDDs complete, systems index updated, config domains consistent, session state current)
4. **UX spec** — `/ux-design` for `design/ux/hud.md` (HUD UI requirements — flagged in the HUD GDD)
5. **Asset specs** — `/asset-spec` per system once the Art Bible is approved (HUD flagged: system:hud-progress-system)
